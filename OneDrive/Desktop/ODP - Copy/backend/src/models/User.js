...
const deletedQueryFilter = function deletedQueryFilter() {
  ...
  if (options.onlyDeleted) { this.where({ isDeleted: true }); return; }
...
const userSchema = new mongoose.Schema(
  {
    ...
    role: { type: String, enum: ['user', 'support', 'finance', 'admin', 'super_admin'], default: 'user' },
    ...
    refreshTokens: {
      type: [{ token: { type: String, required: true }, createdAt: { type: Date, default: Date.now }, expiresAt: { type: Date, required: true } }],
      default: [],
      select: false
    },
...
userSchema.methods.pruneExpiredTokens = function pruneExpiredTokens() {
  const now = new Date();
